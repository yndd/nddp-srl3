...
func initNetworkinstanceMpls(p *yentry.Entry, opts ...yentry.EntryOption) *yentry.Entry {
	children := map[string]yentry.EntryInitFunc{
		"ndk-label-block": initNetworkinstanceMplsNdklabelblock,
		"static-entry":    initNetworkinstanceMplsStaticentry,
	}
	e := &yentry.Entry{
		Name:             "mpls",
		Key:              []string{},
		...
		LeafRefs: []*leafref.LeafRef{
			{
				...
				RemotePath: &gnmi.Path{
					Elem: []*gnmi.PathElem{
						...
						{Name: "mpls"},
						{Name: "label-ranges"},
						{Name: "static", Key: map[string]string{"name": ""}},
					},
				},
			},
		},
