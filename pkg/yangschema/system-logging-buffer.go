...
func initSystemLoggingBuffer(p *yentry.Entry, opts ...yentry.EntryOption) *yentry.Entry {
	children := map[string]yentry.EntryInitFunc{
		"facility":  initSystemLoggingBufferFacility,
		"filter":    initSystemLoggingBufferFilter,
		"subsystem": initSystemLoggingBufferSubsystem,
	}
	e := &yentry.Entry{
		Name: "buffer",
		Key: []string{
