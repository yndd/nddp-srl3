...
func initAcl(p *yentry.Entry, opts ...yentry.EntryOption) *yentry.Entry {
	children := map[string]yentry.EntryInitFunc{
		"capture-filter": initAclCapturefilter,
		"cpm-filter":     initAclCpmfilter,
		"ipv4-filter":    initAclIpv4filter,
		"ipv6-filter":    initAclIpv6filter,
		"policers":       initAclPolicers,
		"system-filter":  initAclSystemfilter,
	}
	e := &yentry.Entry{
		Name:             "acl",
		Key:              []string{},
