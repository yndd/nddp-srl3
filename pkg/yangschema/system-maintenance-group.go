...
func initSystemMaintenanceGroup(p *yentry.Entry, opts ...yentry.EntryOption) *yentry.Entry {
	children := map[string]yentry.EntryInitFunc{
		"maintenance-mode": initSystemMaintenanceGroupMaintenancemode,
		"members":          initSystemMaintenanceGroupMembers,
	}
	e := &yentry.Entry{
		Name: "group",
		Key: []string{
		...
		LeafRefs: []*leafref.LeafRef{
			{
				...
				RemotePath: &gnmi.Path{
					Elem: []*gnmi.PathElem{
						{Name: "system"},
						{Name: "maintenance"},
						{Name: "profile", Key: map[string]string{"name": ""}},
					},
				},
			},
		},
