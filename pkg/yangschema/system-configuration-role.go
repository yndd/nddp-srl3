...
func initSystemConfigurationRole(p *yentry.Entry, opts ...yentry.EntryOption) *yentry.Entry {
	children := map[string]yentry.EntryInitFunc{
		"rule": initSystemConfigurationRoleRule,
	}
	e := &yentry.Entry{
		Name: "role",
		Key: []string{
		...
		LeafRefs: []*leafref.LeafRef{
			{
				...
				RemotePath: &gnmi.Path{
					Elem: []*gnmi.PathElem{
						...
						{Name: "aaa"},
						{Name: "authorization"},
						{Name: "role", Key: map[string]string{"rolename": ""}},
					},
				},
			},
		},
